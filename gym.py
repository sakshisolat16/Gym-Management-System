...
def check_date():
    ...
    for registration in registrations:
        ...
        if datetime.strptime(end_date, '%Y-%m-%d') < current_date:
            ...
            sms_message="Your gym membership has expired. Renew your subscription to continue accessing FitTrack GYM."
...
def send_sms_for_expiration():
    ...
    for registration in registrations:
        ...
        if datetime.strptime(end_date, '%Y-%m-%d') == current_date + timedelta(days=3):
            ...
            # Send SMS to the member
            sms_message="Your gym membership will expire in 3 days. Renew your subscription to continue accessing FitTrack GYM."
            send_sms_notification(contact_no, sms_message)
...
class RegistrationFrame(ctk.CTkFrame):
    def __init__(self, master, data=None, **kwargs):
        ...
        label=ctk.CTkLabel(self, text="FitTrack Gym Membership Registration", font=("Arial bold", 26))
    ...
    def register_subscription(self):
        ...
        sms_message=f"Hello {first_name}!, You have Successfully Subscribed for {subscription_plan} Plan. Subscription ID:{subscription_id}. Start Date: {start_date} End Date: {end_date}, - FitTrack GYM"
            ...
            with sqlite3.connect('SQLite db/attendance_records.db') as conn_attendance:
                ...
                if attendance_type == "Time In":
                    ...
                    self.send_sms(contact_no,
                                  f"Hello {first_name}!, You have Successfully Time In. Subscription ID:{subscription_id}. Time In: {current_datetime}, - FitTrack GYM")
                    ...
                    self.send_sms(contact_no,
                                  f"Hello {first_name}!, Thank you for coming. See you again! Subscription ID:{subscription_id}. Time Out: {current_datetime}, - FitTrack GYM")
...
class TrainerFrame(ctk.CTkFrame):
    ...
    def register_subscription(self):
        ...
        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()  # Assuming contact_no_entry contains the formatted phone number
        sms_message=f"Hello {first_name}!, You are registered as a Trainer in FitTrack Gym."
        self.send_sms(formatted_contact_no, sms_message)
                ...
                if status and status[0] in ['Active', 'On Leave']:
                    ...
                    with sqlite3.connect('SQLite db/trainer_attendance_records.db') as attendance_conn:
                        ...
                        if attendance_type == "Time In":
                            ...
                            send_sms(contact_no,
                                     f"Hello {first_name}!, You have Successfully Time In. Time In: {current_datetime}, - FitTrack GYM")
                            ...
                            send_sms(contact_no,
                                     f"Hello {first_name}!, You have Successfully Time Out. Time In: {current_datetime}, - FitTrack GYM")
...
class LogbookFrame(ctk.CTkFrame):
    ...
    def attend_log(self):
        ...
        try:
            ...
            self.send_sms(contact_no,
                          f"Hello {first_name}!, Thank You for attending FitTrack GYM. Time In: {current_time}")
...
class RegisterEmployeeFrame(ctk.CTkFrame):
    ...
    def register_employees(self):
        ...
        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()  # Assuming contact_no_entry contains the formatted phone number
        sms_message=f"Hello {first_name}!, You are registered as an Employee of FitTrack Gym."
        self.send_sms(formatted_contact_no, sms_message)
...
def register(full_name, contact_no, username, password, contact_no_entry, full_name_entry, username_entry,
    ...
    try:
        ...
        if existing_user:
            ...
            send_sms(contact_no, f"Hello {full_name}!, You have Successfully Registered as user. - FitTrack GYM")
            conn.commit()
...
def create_login_window():
    ...
    center_window(login_window,1240,600 )
